...
- **`src/engine/betting.ts`** - 6 actions: Check, Call, Half-Pot (50% pot), Raise (2x last raise), All-in, Fold. `BetState` tracks `playerActed`/`aiActed` flags for determining when betting is complete. `getBetAmounts()` calculates the silver cost for each action.
